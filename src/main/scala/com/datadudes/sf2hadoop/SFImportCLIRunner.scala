...
object SFImportCLIRunner extends App with LazyLogging {
  ...
  case class Config(command: String = "", sfUsername: String = "", sfPassword: String = "",
                    ...
                    sfWSDL: File = new File("."), stateFile: File = new File("$HOME/.sf2hadoop/state"),
  ...
  val parser = new scopt.OptionParser[Config]("sf2hadoop") {
    ...
    opt[File]('s', "state") optional() valueName "<file>" action { (x, c) => c.copy(stateFile = x)} text "Path to state file to keep track of last updated timestamps"
  ...
  def handleCommand(config: Config) = {
    if(config.command.isEmpty) {
      ...
      if(config.command == "init") {
        createConfigDir
        val existingStates = readStates(DEFAULT_STATE_FILE)
        ...
        saveStates(DEFAULT_STATE_FILE, updatedStates)
      }
      else {
        val existingStates = readStates(DEFAULT_STATE_FILE)
        ...
        saveStates(DEFAULT_STATE_FILE, newStates)
      }
    }
  }
